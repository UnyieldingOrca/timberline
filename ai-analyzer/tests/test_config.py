...
def test_from_cli_overrides():
    ...
    with patch.dict(os.environ, {'LLM_API_KEY': 'env-key'}, clear=True):
        cli_overrides = {
        ...
        assert settings.llm_api_key == 'env-key'  # From environment


def test_validation_empty_milvus_host():
    """Test validation fails for empty Milvus host"""
    config = {'milvus_host': '', 'llm_api_key': 'test-key'}
...
def test_validation_invalid_milvus_port():
    ...
    config = {'milvus_port': 0, 'llm_api_key': 'test-key'}
    with pytest.raises(ValueError, match="Milvus port must be between 1 and 65535"):
        Settings.from_dict(config)

    config = {'milvus_port': 70000, 'llm_api_key': 'test-key'}
    with pytest.raises(ValueError, match="Milvus port must be between 1 and 65535"):
        Settings.from_dict(config)
...
def test_validation_empty_milvus_collection():
    ...
    config = {'milvus_collection': '', 'llm_api_key': 'test-key'}
...
def test_validation_negative_analysis_window():
    ...
    config = {'analysis_window_hours': -1, 'llm_api_key': 'test-key'}
...
def test_validation_negative_max_logs():
    ...
    config = {'max_logs_per_analysis': -1, 'llm_api_key': 'test-key'}
...
def test_validation_negative_cluster_batch_size():
    ...
    config = {'cluster_batch_size': -1, 'llm_api_key': 'test-key'}
...
def test_validation_empty_llm_model():
    """Test validation fails for empty LLM model"""
    config = {'llm_model': '', 'llm_api_key': 'test-key'}
...
def test_validation_empty_report_dir():
    ...
    config = {'report_output_dir': '', 'llm_api_key': 'test-key'}
...
def test_to_dict(valid_settings_dict):
    ...
    assert result['llm_api_key'] == '***'  # Should be masked
...
def test_get_sanitized_dict(valid_settings_dict):
    ...
    assert result['milvus_host'] == 'localhost'
    assert result['llm_api_key'] is None  # Should be None
